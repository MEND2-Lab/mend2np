...
`<cue>` is hardcoded to `{color, shape, lethality}` for switch-cost contrasts (see `_SWITCH_COST_CUES` in [mend2np/sert.py](../mend2np/sert.py)). `<metric>` is hardcoded to `{mean_first_rt, median_first_rt, mean_correct_resp_rt, median_correct_resp_rt, accuracy}` (`_SWITCH_COST_METRICS`). Switch-cost columns where either operand is missing produce `NaN` — common at the per-cue trial level for short runs that lack a switch (or repeat) trial of a given cue.

`num_correct` is deliberately **not** among the switch-cost metrics. Switch and repeat buckets rarely contain the same number of trials — at the trial level a typical run has roughly half as many switch trials as repeat trials — so a difference of raw counts would mostly reflect that imbalance rather than a switch cost. Use `<...>_switch_cost_<...>_accuracy`, the rate-normalized contrast, instead. Per-bucket `_num_correct` values are still emitted for every bucket.

Note that switch-cost columns are signed differences, so **negative values are expected and meaningful**: a negative RT switch cost means that participant responded faster on switch trials than on repeat trials, and a negative accuracy switch cost means they were less accurate on switch trials.
